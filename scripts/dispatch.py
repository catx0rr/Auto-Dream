#!/usr/bin/env python3
"""
autodream: dispatch — mode dispatch engine

Reads autodream.json, compares lastRun timestamps against current time,
returns which dream modes are due. Also handles lastRun updates after a cycle.

Usage:
    python3 dispatch.py --config ~/.openclaw/autodream/autodream.json
    python3 dispatch.py --config ~/.openclaw/autodream/autodream.json --update-lastrun core,rem
"""
...
def main():
    parser = argparse.ArgumentParser(description='AutoDream: Mode Dispatch')
    parser.add_argument('--config', required=True, help='Path to autodream.json')
    ...
    return 0 if result['due_modes'] else 1

#!/usr/bin/env python3
"""
autodream: stale — stale thread detection

Scans LTMEMORY.md Open Threads for items not referenced in N+ days.
Cross-references with index.json and daily logs for last-mention dates.

Usage:
    python3 stale.py --memory-file LTMEMORY.md --index memory/index.json --threshold 14
"""
...
def find_last_mention(thread_text: str, log_dir: str, index_path: str) -> str:
    ...
    # Check index.json for lastReferenced
...
def main():
    parser = argparse.ArgumentParser(description='AutoDream: Stale Thread Detection')

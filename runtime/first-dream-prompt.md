...
**Config:** Ensure `~/.openclaw/autodream/autodream.json` exists. If not, create from `references/memory-template.md` template. Set `promotedBy: "first-dream"` on all entries created during this run.
...
```
MEMORY_LINES = wc -l LTMEMORY.md (0 if missing)
MEMORY_SECTIONS = grep -c "^## " LTMEMORY.md (0 if missing)
...
Skip small talk. Skip content already in LTMEMORY.md that hasn't changed.

## Phase 3: Consolidate

Read LTMEMORY.md. Compare with extracted content:

- **New** → append to appropriate LTMEMORY.md section
...
Update `_Last updated:` date in LTMEMORY.md.
...
```
MEMORY_LINES_AFTER = wc -l LTMEMORY.md
...
📦 Your memory assets:
   ...
   • {MEMORY_LINES} lines of long-term memory (LTMEMORY.md)
...
If this is a brand new instance with no daily logs and minimal LTMEMORY.md:
...
- Backup: if LTMEMORY.md changes >30%, save LTMEMORY.md.bak first

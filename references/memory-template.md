...
| `modes.{name}.minUnique` | number | — | Minimum uniqueSessionCount to qualify |
...
3. Add `promotedBy` — set to `"core"` for all existing entries
4. Move `config` block from index.json to `~/.openclaw/autodream/autodream.json`
5. Add `gateStats` to stats block
...
└── memory/
    ...
    ├── index.json                   # Entry metadata + stats (v3.1)
